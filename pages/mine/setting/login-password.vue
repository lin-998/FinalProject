...
<!-- 			<u-tabs :list="list" :bold="false" :is-scroll="false" font-size="28" active-color='#00B9FF' inactive-color="#999"
		...
		<view class="loginpwd">
			<view class="LittleBg loginpwd-in">
				...
				<view class="phone">
					<u-input class="input" v-model="loginPwd" placeholder="8-12位字母+数字组合" type="password" />
				</view>
			...
			<u-button class="logout submitBtn" type="primary" v-show="current==0">确认</u-button>
	...
	export default {
		...
		methods: {
			...
			verificationCode(){
				...
				}else if(!((/\d+/.test(this.loginPwd)) && (/[a-zA-Z]+/.test(this.loginPwd)))){
					return this.$toast('密码至少包含一个数字跟字母')
				}else if(this.loginPwd.length<8||this.loginPwd.length>12){
					return this.$toast('密码长度不得小于8位或者大于12位')
				}else if(this.loginPwd!=this.newloginPwd){
			...
			//重置密码
			updetaPwd(){
				mineApi.updateLoginPwd({
					userId:this.userInfo.id,
					phone:this.userInfo.phone,
					newLoginPassword:this.loginPwd,
			...
			getLoginCode(){
				...
				mineApi.LoginPwdSms({phone:this.userInfo.phone}).then(res=>{
					console.log(res);
					if(res.code==200){
						this.show = false
						this.$toast('短信发送成功');
